...
void JetObservablesRetriever::fill_track_IP(const edm4hep::ReconstructedParticle& jet,
  ...
  const float sip2d = std::copysign(
      ...
      part_pnt.X() * jet_p.X() +
          part_pnt.Y() *
              jet_p
                  .Y());  // dot product between part and jet in 2D: if angle between track and jet greater 90 deg -> negative sign; if smaller 90 deg -> positive sign
  ...
  const float sip3d = std::copysign(
      std::abs(IP_3d),
      part_pnt.Dot(
          jet_p));  // dot product between part in jet in 3d: if angle between track and jet greater 90 deg -> negative sign; if smaller 90 deg -> positive sign
