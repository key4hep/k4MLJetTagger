...
echo "time before running script: $middle_time seconds" # 453 sec for 4 files 

echo "running script ..."

# CHANGE FILE HERE
k4run /afs/cern.ch/work/s/saaumill/public/JetTagging/k4MLJetTagger/options/writeJetConstObs.py --inputFiles="./job/merged_input_${pattern}.root" --outputFile="./job/out.root" 
#k4run /afs/cern.ch/work/s/saaumill/public/JetTagging/k4MLJetTagger/options/writeJetTags.py --inputFiles="./job/merged_input_${pattern}.root" --outputFile="./job/out.root"

echo "job done ... "
job_endtime=$(date +%s)
job_time=$((job_endtime - preproc_time)) 
echo "time to run job: $job_time seconds"  # 1171 sec for 4 files (with 1000 events each)
# make directory if it does not exist:
if [ ! -d ${OUTPUT_DIR} ]; then
  mkdir -p ${OUTPUT_DIR}
fi

echo "copying output file to: ${OUTPUT_FILE}"
